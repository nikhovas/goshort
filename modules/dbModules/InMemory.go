...
func CreateInMemory(kernel *kernel.Kernel) types.DatabaseInterface {
...
func (controller *InMemory) Run(wg *sync.WaitGroup) error {
	wg.Done()
...
func (controller *InMemory) Delete(key string) error {
	url, err := controller.Get(key)
	if err != nil {
		return err
	}
	...
	if url.Autogenerated {
		...
		controller.dataStorage.Delete(url.Key)
		controller.inverseStorage.Delete(url.Url)
	} else {
		controller.mutex.RLock()
		controller.mutex.RUnlock()
